...
router.put("/profile", authMiddleware, upload.single("profileImage"), async (req, res) => {
  try {
    ...
    const { bio } = req.body;
    const newProfileImage = req.file ? `/uploads/profiles/${req.file.filename}` : null;
    ...
    if (bio !== undefined) {
      if (bio.length > 160) {
        return res.status(400).json({ message: "자기소개는 160자를 초과할 수 없습니다." });
      }
      query += "bio = ?, ";
      params.push(bio);
    }
    ...
    if (oldProfileImagePath) {
      const oldImageFilePath = path.join(new URL('.', import.meta.url).pathname, '..', oldProfileImagePath).substring(1);
