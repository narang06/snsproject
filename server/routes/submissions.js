...
router.get("/feed", authMiddleware, async (req, res) => {
  try {
    ...
    const limit = parseInt(req.query.limit || "5", 10); // 한 페이지에 5개씩
    ...
    const [submissions] = await db.query(sqlQuery, [
      req.user.userId,
      req.user.userId,
      ...queryParams,
      limit,
  ...
  async (req, res) => {
    try {
      ...
      if (!req.files || req.files.length === 0) {
        return res
          .status(400)
          .json({ message: "퀘스트 인증을 위해 이미지는 필수입니다." });
      }

      const today = new Date().toISOString().split("T")[0];
      const imageUrls = req.files.map(
        (file) => `/uploads/submissions/${today}/${file.filename}`,
      );
      ...
      if (content.trim().length < 10 || content.trim().length > 500) {
        return res
          .status(400)
          .json({ message: "내용은 10자 이상 500자 이하로 입력해주세요." });
      }
      ...
      for (const recipientId of recipients) {
        await db.query(
          ...
          [recipientId, userId, "mention", "SUBMISSION", submissionId],
        );
      }
      // --- 언급 알림 로직 종료 ---
      ...
      try {
        ...
        if (Array.isArray(images)) {
          images.forEach((imageUrl) => {
            const filePath = path
              .join(new URL(".", import.meta.url).pathname, "..", imageUrl)
              .substring(1);
...
router.put(
  ...
  async (req, res) => {
    try {
      ...
      // 2. 삭제할 이미지 처리
      const imagesToDeleteParsed = imagesToDelete
      ...
      // 3. 최종 이미지 목록 생성
      const existingImages = submissions[0].content_image_url
        ? JSON.parse(submissions[0].content_image_url)
        : [];
      const remainingImages = existingImages.filter(
        (url) => !imagesToDeleteParsed.includes(url),
      );
      ...
      if (finalImageUrls.length === 0) {
        return res
          .status(400)
          .json({ message: "이미지는 최소 1개 이상 필요합니다." });
      }
      if (finalImageUrls.length > 5) {
        return res
          .status(400)
          .json({ message: "이미지는 최대 5개까지 가능합니다." });
      }
